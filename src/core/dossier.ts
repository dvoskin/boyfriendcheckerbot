...
export async function writeDossier(graph: GraphResult): Promise<Dossier> {
  ...
  const msg = await client.messages.create({
    ...
    system: [
      ...
      '',
      'Include a one-line NAMESAKE-RISK read near the end — low / medium / high — based on whether',
      'the identity looks confirmed (a name+city+age all lining up = low; a common name with no',
      'city and scattered records = high). It tells the user how much to trust the match itself.',
      '',
