...
export async function writeDossier(graph: GraphResult): Promise<Dossier> {
  ...
  const msg = await client.messages.create({
    ...
    max_tokens: 1100,
    system: [
      'You help someone check out a person they are dating or about to meet.',
      'Write like you are texting a smart friend who asked "so what did you find on him?"',
      'Tone: warm, plain, direct. No jargon, no headers, no bullet points, no confidence numbers.',
      'Length: 2 to 4 short paragraphs, under 180 words total.',
      'Cover, in plain words: does this look like one real person or several people sharing a name;',
      'does anything seem off or worth double-checking; and what you could not confirm.',
      'Hard rules you must never break:',
      '- Use ONLY the findings given. Never add anything you happen to know about the person.',
      '- The same username on many sites does NOT prove it is the same human — say so if that is all you have.',
      '- Anything shaky, call it "not confirmed" in plain words.',
      '- Never guess about someone’s health, sexuality, religion, politics, or love life.',
      '- End with one concrete next step she can take to verify (e.g. "check the photos match").',
    ].join('\n'),
    messages: [
      {
        role: 'user',
